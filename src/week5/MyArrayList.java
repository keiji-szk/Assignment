...
public class MyArrayList<E> implements List<E> {
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        ...
        Object[] a = c.toArray();
        Object[] newElmData = new Object[size + a.length];
        ...
        System.arraycopy(elementData, index,
                newElmData, index + a.length,
                size - index);

        elementData = newElmData;
        size += a.length;
        return true;
    }
    ...
    public boolean retainAll(Collection<?> c) {
        int cntCopy = 0;
        Object[] tmpElmData = new Object[size];
        HashSet<?> cSet = new HashSet<>(c);
        for (Object e : elementData) {
            if (!cSet.contains(e))
                continue;
            tmpElmData[cntCopy++] = e;
        }

        if (cntCopy == size)
            return false;

        elementData = new Object[cntCopy];
        System.arraycopy(tmpElmData, 0, elementData, 0, cntCopy);
        size = cntCopy;
        return true;
    }
